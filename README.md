...
> [!IMPORTANT]  
> For artifact review, refer to the `./ARTIFACT-APPENDIX.md` file.
> After review, the contents will be merged with the (currently) outdated README.
...
_Note: In the current Android Developer Previews, Executor apps must be installed as **Privileged System Apps** to execute AppFunctions._

_Note 2: The executor application requires a valid Google AI Studio API key for Gemini models. Used in `app/ui/screen/AgentChatViewModel.kt`_
...
_Note: the tool application can be run as-is in Android Studio. It requires no elevated permissions or specific API keys._
