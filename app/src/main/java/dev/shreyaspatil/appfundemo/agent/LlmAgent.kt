            ...
            val result = runCatching {
                when {
                    ...
                    parsed.functionId.endsWith("getMAC") -> {
                        // API access gives default 02:00..., since this demo is on a rooted phone we cat wlan0 directly
                        try {
                            val process = Runtime.getRuntime().exec(arrayOf(
                                "su", "-c", "cat /sys/class/net/wlan0/address"
                            ))
                            process.inputStream.bufferedReader().readLine()
                                ?.trim()?.takeIf { it.isNotBlank() } ?: "unavailable"
                        } catch (e: Exception) {
                    ...
                    parsed.functionId.endsWith("getPhoneNumber") -> {
                        // API does not return phoneNumber - bypass via cmd
                        try {
                            val process = Runtime.getRuntime().exec(arrayOf(
                                "su", "-c",
                                "content query --uri content://telephony/siminfo --projection number"
                            ))
                            val output = process.inputStream.bufferedReader().readText()
                            val match = Regex("number=([0-9+]+)").find(output)
                            match?.groupValues?.get(1)?.takeIf { it.isNotBlank() } ?: "unavailable"
                        } catch (e: Exception) {
                            "unavailable: ${e.message}"
                        }
                    }
                    ...
                    parsed.functionId.endsWith("getEmail") -> {
                        // API does not return accounts email - bypass via cmd. We are acting as a
                        // system privileged app, so fits the use-case
                        try {
                            val process = Runtime.getRuntime().exec(arrayOf(
                                "su", "-c", "dumpsys account"
                            ))
                            val lines = process.inputStream.bufferedReader().readLines()
                            lines.firstOrNull { it.contains("type=com.google") }
                                ?.substringAfter("name=")
                                ?.substringBefore(",")
